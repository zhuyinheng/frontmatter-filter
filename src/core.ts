...
async function buildPublishPlan(
  ...
  const selection = derivePublicationSelection(sourceTree, config.brokenLinkPolicy);

  const { publishedMarkdown, copiedAttachments: attachmentFiles, brokenLinks, warnings } = selection;
...
async function collectSourceTree(repoRoot: string, sourceCommit: string): Promise<SnapshotTree> {
  ...
  for (const entry of entries) {
    ...
    if (isReadmeBasename(markdownCandidate.basename)) {
